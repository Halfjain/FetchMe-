...
library(Hmisc)
library(neuralnet)
ft<-read.csv(file="C:/Users/kangbok/Dropbox/Teaching/PM/Spring 2019/BUAL 5600/Fetchme/fetch.csv", header=TRUE, sep=",")

we<-read.csv(file="C:/Users/kangbok/Dropbox/Teaching/PM/Spring 2019/BUAL 5600/Fetchme/weather.csv", header=TRUE, sep=",")
...
colnames(ft)[colnames(ft)=="Breakfast.Ratio..orders.to.drivers."] <- "br" 
colnames(ft)[colnames(ft)=="Lunch.Ratio..orders.to.drivers."] <- "lr" 
colnames(ft)[colnames(ft)=="Dinner.Ratio..orders.to.drivers."] <- "dr" 

ft$br<-as.numeric(ft$br)
ft$lr<-as.numeric(ft$lr)
ft$dr<-as.numeric(ft$dr)


ft$lag.br<-Lag(ft$br, +1)
ft$lag.lr<-Lag(ft$lr, +1)
ft$lag.dr<-Lag(ft$dr, +1)
...
ft$abt<-as.numeric(ft$abt)
ft$alt<-as.numeric(ft$alt)
ft$adt<-as.numeric(ft$adt)
ft$addt<-as.numeric(ft$addt)


ft$abt[ft$abt=="-"]<-"0"
ft$abt<-as.numeric(ft$abt)

ft$lag.abt<-Lag(ft$abt, +1)
ft$lag.alt<-Lag(ft$alt, +1)
ft$lag.adt<-Lag(ft$adt, +1)
ft$lag.addt<-Lag(ft$addt, +1)
...
# regression with a lagged depdent and weather condition
...
fm0<-lm(Total.Order~wc.t2+wc.t3, data=trainset)
summary(fm0)
yhat0<-predict(fm0, new=testset)
er0 <- yhat0-testset$Total.Order 
mean(er0^2); mean(abs(er0))
...
# regression with a lagged dependent + dates and lagged driver ratio
fm4<-lm(Total.Order~lag.y1+as.factor(date)+
          lag.br+lag.lr+lag.dr, data=trainset)
summary(fm4)
yhat4<-predict(fm4, new=testset)
er4 <- yhat4-testset$Total.Order
mean(er4^2); mean(abs(er4))

# regression with a lagged dependent + dates and lagged deliverty time
fm4<-lm(Total.Order~lag.y1+as.factor(date)+
          lag.abt+lag.alt+lag.adt, data=trainset)
summary(fm4)
yhat4<-predict(fm4, new=testset)
er4 <- yhat4-testset$Total.Order
mean(er4^2); mean(abs(er4))

# regression with a lagged dependent + dates, lagged driver ratio and lagged 
# delivery time
fm4<-lm(Total.Order~lag.y1+as.factor(date)+
          lag.br+lag.lr+lag.dr+lag.abt+lag.alt+lag.adt, data=trainset)
summary(fm4)
yhat4<-predict(fm4, new=testset)
er4 <- yhat4-testset$Total.Order
mean(er4^2); mean(abs(er4))

# regression with a lagged dependent + dates, lagged delivery time
fm4<-lm(Total.Order~lag.y1+as.factor(date)+
          lag.abt+lag.alt+lag.adt, data=trainset)
summary(fm4)
yhat4<-predict(fm4, new=testset)
...
# regression with a lagged dependent + dates, lagged delivery time 
# and averaged delivery time
fm4<-lm(Total.Order~lag.y1+as.factor(date)+
          lag.abt+lag.alt+lag.adt+addt, data=trainset)
summary(fm4)
yhat4<-predict(fm4, new=testset)
er4 <- yhat4-testset$Total.Order
mean(er4^2); mean(abs(er4))
